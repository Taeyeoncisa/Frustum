...
func Elastico() {
	...
	commitee_time := float64(time.Since(startTime).Seconds())
	// // 2.m个委员会各自运行
	...
	// 	<-runPBFT
	// }
	PBFT(nodes_iter, committeeList[0])
	run_time := float64(time.Since(startTime).Seconds())
	fmt.Printf("%.8f\n", commitee_time/run_time)
}
...
func PBFT(nodes []Node, commiteeMember []Node) {
