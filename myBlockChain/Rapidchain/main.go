...
var m sync.Mutex
var countCrossTx chan bool = make(chan bool, 10000)
...
func addTx2pool(tx Transaction, wg *sync.WaitGroup, m *sync.Mutex) {
	m.Lock()
	// fmt.Printf("tx: %v\n", tx)
...
func verifyCrossTx(block ProposedBlock) {
	for _, t := range block.Transactions {
		if nodes[t.Inputs].committeeID !=
			nodes[t.Outputs].committeeID {
			countCrossTx <- true
			// 拆分交易
			t1 := Transaction{
				Hash:         [32]byte{},
			...
			w.Add(1)
			addTx2pool(t1, &w, &m)
			// fmt.Printf("len(countCrossTx): %v\n", len(countCrossTx))
		} else if t.OriginalHash != [32]byte{} && t.Hash == [32]byte{} {
			t.Hash = t.calculateHash()
			w.Add(1)
			addTx2pool(t, &w, &m)
		} else if t.OriginalHash != [32]byte{} && t.Hash != [32]byte{} {
			t_new := t
			t_new.OriginalHash = [32]byte{}
			w.Add(1)
			addTx2pool(t_new, &w, &m)
		}
	}
}
...
func main() {
	...
	for i := 1; i <= int(flagArgs.n); i++ {
		...
		// fmt.Printf("nodes[i].committeeID: %v\n", nodes[i].committeeID)
	}
	// 6.用户验证交易
